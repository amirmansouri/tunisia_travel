...
export default function ReservationImport() {
  ...
  const handleDownloadTemplate = () => {
    window.location.href = '/api/admin/reservations/template';
  };
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/reservations/import', {
        method: 'POST',
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        setResult({
          type: 'success',
          message: data.message || `Imported ${data.imported} reservations`,
        });
